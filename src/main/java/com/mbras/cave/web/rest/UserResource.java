...
import com.mbras.cave.config.Constants;
import com.codahale.metrics.annotation.Timed;
import com.mbras.cave.domain.Cellar;
import com.mbras.cave.domain.User;
import com.mbras.cave.repository.UserRepository;
import com.mbras.cave.repository.search.UserSearchRepository;
import com.mbras.cave.security.AuthoritiesConstants;
import com.mbras.cave.service.CellarService;
import com.mbras.cave.service.MailService;
import com.mbras.cave.service.UserService;
import com.mbras.cave.web.rest.vm.ManagedUserVM;
import com.mbras.cave.web.rest.util.HeaderUtil;
import com.mbras.cave.web.rest.util.PaginationUtil;
...
public class UserResource {
    ...
    /**
     * GET  /users/:login/cellars : get the "login" user.
     *
     * @param login the login of the user to find
     * @return the ResponseEntity with status 200 (OK) and with body the "login" user, or with status 404 (Not Found)
     */
    @RequestMapping(value = "/users/{login:" + Constants.LOGIN_REGEX + "}/cellars",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Cellar> getCellarForUser(@PathVariable String login) {
        log.debug("REST request to get User : {}", login);
        Cellar cellar = cellarService.findByUser(login);
